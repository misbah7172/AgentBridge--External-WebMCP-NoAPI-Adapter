...
if(document.modelContext){const c=new AbortController,A=new Set(['get_cart','add_to_cart','update_cart','remove_from_cart','clear_cart','get_wishlist','add_to_wishlist','remove_from_wishlist','get_orders','get_order_details','cancel_order','apply_coupon','checkout']);const R=async()=>{let a=false;try{const r=await fetch('/api/auth/session',{credentials:'same-origin'}),j=await r.json();a=!!(j&&j.success&&j.data&&j.data.user)}catch{}for(const [name,description,inputSchema,execute] of T)if(a||!A.has(name))await document.modelContext.registerTool({name,description,inputSchema,execute},{signal:c.signal})};R();addEventListener('pagehide',()=>c.abort(),{once:true})}`;
